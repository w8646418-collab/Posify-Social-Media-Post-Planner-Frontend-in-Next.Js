...
const AutoPosting = () => {
  ...
  const fetchScheduledPosts = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:5000/api/schedule/get_scheduledpost?email=${userEmail}`
      );
  ...
  const handleDelete = async (index) => {
    ...
    try {
      const res = await fetch(
        "http://localhost:5000/api/schedule/delete_scheduledpost",
        {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
    ...
    <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-10">
        ...
        <div className="bg-white shadow-lg rounded-2xl p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-linear-to-r from-purple-600 to-purple-800 text-white py-3.5 rounded-lg font-semibold hover:scale-105 transition-transform shadow-md"
            >
              {loading
                ? "Saving..."
                : editPostId !== null
      ...
      <div className="md:w-1/2 bg-white rounded-xl p-6 shadow-sm border border-gray-200">
        ...
        {scheduledPosts.length === 0 && (
          <p className="text-center text-gray-400 py-10">No posts scheduled</p>
        )}
